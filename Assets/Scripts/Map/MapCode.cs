...
public class MapReader
{
    ...
    public int PixelWidth
    {
        get
        {
            return Width * MapTools.CellWidth;
        }
    }
    ...
    public int PixelHeight
    {
        get
        {
            return Height * MapTools.CellHeight;
        }
    }
    ...
    private void initiate()
    {
        ...
        int temp1 = Convert.ToInt32("0x43", 16);
        int temp2 = Convert.ToInt32("0x23", 16);
        byte temp3 = Bytes[2];
        byte temp4 = Bytes[2];
            ...
            for (int x = 0; x < Width; x++)
                ...
                {//14
                    ...
                    if (MapCells[x, y].BackIndex > 100)
                    {
                        ...
                        {
                            BackIndex = MapCells[x, y].BackIndex;
                            UnityEngine.Debug.LogError(FileName + " BackIndex index: " + MapCells[x, y].BackIndex);
                        }
                    }

                    if (MapCells[x, y].FrontIndex > 120)
                    {
                        if (FrontIndex != MapCells[x, y].FrontIndex)
                        {
                            FrontIndex = MapCells[x, y].FrontIndex;
                            UnityEngine.Debug.LogError(FileName + " FrontIndex index: " + MapCells[x, y].FrontIndex);
                        }
                    }

                    if (MapCells[x, y].MiddleIndex > 110)
                    {
                        if (MiddleIndex != MapCells[x, y].MiddleIndex)
                        {
                            MiddleIndex = MapCells[x, y].MiddleIndex;
                            UnityEngine.Debug.LogError(FileName + " MiddleIndex index: " + MapCells[x, y].MiddleIndex);
                        }
                    }
                }
        }
        ...
        if (File.Exists(FileName))
        {
            using (FileStream fileStream = new FileStream(FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                using (BinaryReader binaryReader = new BinaryReader(fileStream))
                {
                    ...
                    byte[] TitleBytes = binaryReader.ReadBytes(17);
                    byte[] UpdateTimeBytes = binaryReader.ReadBytes(8);
                    byte[] ReversedBytes = binaryReader.ReadBytes(23);
                    binaryReader.BaseStream.Seek(52, SeekOrigin.Begin);
                    for (int i = 0; i < (int)Width; i++)
                    {
                        for (int j = 0; j < (int)Height; j++)
                        {
                            MapCells[i, j] = this.ReadCell(binaryReader);
                        }
                    }
                }
            }
    ...
    protected virtual CellInfo ReadCell(BinaryReader reader)
    {
        CellInfo mapCell = new CellInfo();
        mapCell.BackIndex = 0;
        mapCell.MiddleIndex = 0;
        ushort num = reader.ReadUInt16();
        mapCell.BackImage = (int)(num & 32767);
        ushort num2 = reader.ReadUInt16();
        mapCell.MiddleImage = (short)(num2 & 32767);
        ushort num3 = reader.ReadUInt16();
        mapCell.FrontImage = (short)(num3 & 32767);
        byte b = reader.ReadByte();
        mapCell.DoorIndex = (byte)(b & 127);
        byte b2 = reader.ReadByte();
        mapCell.DoorOffset = (byte)(b2 & 127);
        mapCell.FrontAnimationFrame = reader.ReadByte();
        mapCell.FrontAnimationTick = reader.ReadByte();
        mapCell.FrontIndex = (short)reader.ReadByte();
        mapCell.Light = reader.ReadByte();
        return mapCell;
    }
